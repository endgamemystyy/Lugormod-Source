...
void G_Damage(gentity_t *targ, gentity_t *inflictor, gentity_t *attacker, vec3_t dir, vec3_t point, int damage, int dflags, int mod) {
	...
	if (attacker && attacker->inuse && attacker->client && attacker->s.number < MAX_CLIENTS)
	{
		if (p_playerInfo[attacker->s.number].cloaked >= level.time)
		{
			CloakStop(attacker);
		}
		if (p_playerInfo[attacker->s.number].forceshield >= level.time)
		{
			damage /= 2;
		}
	}

	if (targ && targ->inuse && targ->client && targ->s.number < MAX_CLIENTS) {
		if (attacker && attacker->inuse && attacker->client && attacker->s.number < MAX_CLIENTS) {
			if (p_playerInfo[targ->s.number].cloaked >= level.time)
			{
				p_playerInfo[targ->s.number].cloakdmg += damage;
				if (p_playerInfo[targ->s.number].cloakdmg >= 25)
				{
					CloakStop(targ);
					p_playerInfo[targ->s.number].cloakdmg = 0;
				}
			}
		}
	}
	...
	*/
	...
	LmdAbility::g_damage(targ, attacker, mod);
