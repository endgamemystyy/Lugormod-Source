...
void lmd_playercheck_use(gentity_t *self, gentity_t *other, gentity_t *activator){
	if (self->Lmd.fireCount > 0) {
		bool targetFired = false;

		for (int i = 0; i < self->Lmd.fireCount; i++) {
			if (PlayerUseableCheck(self, activator)) {
				G_UseTargets(self, activator);
				targetFired = true;
			}
...
const entityInfoData_t lmd_playercheck_keys[] = {
	...
	{ "FireCount", "How many times to fire the target." },
	NULL
};
...
void lmd_playercheck(gentity_t *ent){
	PlayerUsableGetKeys(ent);
	G_SpawnInt("firecount", "", &ent->Lmd.fireCount);
...
void lmd_remove_iobject_use(gentity_t* self, gentity_t* other, gentity_t* activator) {
	if (!self->Lmd.UseReq.prop)
		return;

	if (!activator->client->pers.Lmd.account)
		return;

	Inventory_Destroy(activator, self->Lmd.UseReq.prop);
}

void lmd_remove_iobject(gentity_t* ent) {
	G_SpawnString("property", "", &ent->Lmd.UseReq.prop);
	ent->use = lmd_remove_iobject_use;
}
