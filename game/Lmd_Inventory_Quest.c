...
void Inventory_DownCount_Add_InfoString(gentity_t *ent, iObject_t *obj, int maxEntries) {
	...
	Q_strcat(infostring, sizeof(infostring), va("\"%s\",\"%s\",", "name", obj->fields.name));
	Q_strcat(infostring, sizeof(infostring), va("\"%s\",\"%s\",", "property", data->prop));

	if (maxEntries == 0) {
		Q_strcat(infostring, sizeof(infostring), va("\"%s\",\"%d\",", "maxentries", data->count));
	}
	else {
		Q_strcat(infostring, sizeof(infostring), va("\"%s\",\"%d\",", "maxentries", maxEntries));
	}

	//Q_strcat(infostring, sizeof(infostring), va("\"%s\",\"%d\",", "count", data->count));
	//Q_strcat(infostring, sizeof(infostring), va("\"%s\",\"%d\",", "noAutoDelete", data->noAutoDelete));
...
void Inventory_Destroy(gentity_t *player, char *prop) {
	int i;
	iObjectList_t *inventory = Inventory_Player_GetInventory(player);
	if (!inventory)
		return;
