...
void BattleMedDo(gentity_t* ent, float distance)
{
	...
	org[2] -= 10;
	G_PlayEffectID(G_EffectIndex("misc/possession"), org, ent->client->ps.viewangles);
	for (int i = 0; i < MAX_CLIENTS; i++)
	{
		gentity_t* target = &g_entities[i];
		if (!target || !target->client || !target->inuse || !ForcePowerUsableOn(ent, target, FP_TEAM_HEAL)
			|| target == ent || target->health <= 0 || !isBuddy(ent, target) || target->client->sess.spectatorState)
			continue;

		float cur = Distance(ent->client->ps.origin, target->client->ps.origin);
		if (cur <= distance)
		{
			Account_t* acc = target->client->pers.Lmd.account;
			if (!acc || Accounts_Prof_GetProfession(acc) != 4)
			{
				target->client->ps.fd.forcePower = 100;
				target->client->ps.powerups[PW_FORCE_BOON] = level.time + 30000;
				target->client->ps.powerups[PW_YSALAMIRI] = 0;
			}
			else
			{
				Add_Ammo(target, AMMO_BLASTER, 600);
				Add_Ammo(target, AMMO_POWERCELL, 600);
				Add_Ammo(target, AMMO_METAL_BOLTS, 600);
				target->client->ps.powerups[PW_YSALAMIRI] = level.time + 30000;
				target->client->ps.jetpackFuel = 100;
			}
			p_playerInfo[i].battlemedfx = level.time + 30000;
			...
			target->client->ps.powerups[PW_BATTLESUIT] = level.time + 30000;
			vec3_t pos;
			VectorCopy(target->client->ps.origin, pos);
			pos[2] -= 10;
			G_PlayEffectID(G_EffectIndex("force/heal2"), pos, target->client->ps.viewangles);
			trap_SendServerCommand(target->s.number, va("print \"^7%s ^4has used Battle Meditation on you!\n\"", ent->client->pers.netname));
		}
	}
}
...
void ClientThink_real( gentity_t *ent ) {
	...
	// sskills
	if (p_playerInfo[ent->s.number].thinkDelay + 200 <= level.time) {
		p_playerInfo[ent->s.number].thinkDelay = level.time;

		if (p_playerInfo[ent->s.number].battlemedfx >= level.time)
		{
			if (ent->health <= 0) {
				p_playerInfo[ent->s.number].battlemedfx = 0;
			}
			else {
				ent->health += 4;
				...
				ent->client->ps.stats[STAT_ARMOR] += 2;
				if (ent->client->ps.stats[STAT_ARMOR] > ent->client->ps.stats[STAT_MAX_HEALTH])
					ent->client->ps.stats[STAT_ARMOR] = ent->client->ps.stats[STAT_MAX_HEALTH];
			}
		}

		if (p_playerInfo[ent->s.number].battlemed == 1)
		{
			if (ent->client->ps.torsoAnim != BOTH_MEDITATE || ent->client->ps.legsAnim != BOTH_MEDITATE)
			{
				Disp(ent, "^1You have stopped meditating!");
				p_playerInfo[ent->s.number].battlemed = 0;
				p_playerInfo[ent->s.number].battlemedtimer = 0;
			}
			if (p_playerInfo[ent->s.number].battlemedtimer >= 3000)
			{
				BattleMedDo(ent, 512);
				p_playerInfo[ent->s.number].battlemed = 0;
				p_playerInfo[ent->s.number].battlemedtimer = 0;
			}
			p_playerInfo[ent->s.number].battlemedtimer += 200;
		}

		if (p_playerInfo[ent->s.number].regenSlowed)
		{
			if (p_playerInfo[ent->s.number].regenSlowed >= level.time)
			{
				if (irand(0, 2))
					return;
			}
			...
			{
				G_PlayEffectID(G_EffectIndex("repeater/concussion"), ent->client->ps.origin, ent->client->ps.viewangles);
				p_playerInfo[ent->s.number].regenSlowed = 0;
			}
		}
		if (p_playerInfo[ent->s.number].poisoned)
		{
			if (p_playerInfo[ent->s.number].poisoned >= level.time)
			{
				if (p_playerInfo[ent->s.number].poisonCheckTime <= level.time)
				{
					G_Damage(ent, GetEnt(p_playerInfo[ent->s.number].poisonedBy), GetEnt(p_playerInfo[ent->s.number].poisonedBy),
						NULL, NULL, irand(9, 10), DAMAGE_NO_ARMOR | DAMAGE_NO_PROTECTION, MOD_SLIME);
					G_PlayEffectID(G_EffectIndex("noghri_stick/gas_cloud"), ent->client->ps.origin, ent->client->ps.viewangles);
					p_playerInfo[ent->s.number].poisonCheckTime = level.time + 1000;
				}
			}
			else
			{
				p_playerInfo[ent->s.number].poisoned = 0;
				p_playerInfo[ent->s.number].poisonedBy = -1;
			}
		}	
		if (p_playerInfo[ent->s.number].cloaked)
		{
			if (p_playerInfo[ent->s.number].cloaked >= level.time)
			{
				if (p_playerInfo[ent->s.number].cloakCheckTime <= level.time)
				{
					...
					p_playerInfo[ent->s.number].cloakCheckTime = level.time + (ent->client->isHacking ? 200 : 1000);
				}
			}
			else
			{
		...
		if (p_playerInfo[ent->s.number].repulse)
		{
			if (p_playerInfo[ent->s.number].repulse >= level.time)
			{
				if (p_playerInfo[ent->s.number].repulseCheckTime <= level.time)
				{
					Repulse(ent, qtrue, 32, 128);
					p_playerInfo[ent->s.number].repulseCheckTime = level.time + 200;
				}
			}
			else
			{
				Repulse(ent, qfalse, 192, 64);
				p_playerInfo[ent->s.number].repulse = 0;
			}
		}
	}

	if (p_playerInfo[ent->s.number].screamtimer >= level.time)
	{
		ForceScreamDo(ent, 256);
	}
	//Ufo:
